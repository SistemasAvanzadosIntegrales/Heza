...
class PagoProvisionalpmController extends Zend_Controller_Action {
		...
		if ($periodo == 1) 
			...
			// $query = "
				// SELECT isr_retenido 
				 ...
				 // WHERE id_empresa = '".$empresa."' 
					   // AND id_ejercicio = '".$ejercicio."'
					   // AND id_periodo = '".($periodo - 1)."'";
			
			// $isr_retenido       = $con->execute($query)->fetchAll();
			// $isr_retenido       = ($isr_retenido[0]['isr_retenido']) ? $isr_retenido[0]['isr_retenido'] : 0;
			// $isr_impuesto_pagar = $this->get_isr_impuesto_pagar ($empresa, $ejercicio, $periodo - 1);
			...
			if ($periodo == 2)
				...
				// $isr_retenido_2       = $con->execute($query)->fetchAll();
				// $isr_retenido_2       = ($isr_retenido_2[0]['isr_retenido']) ? $isr_retenido_2[0]['isr_retenido'] : 0;
				$isr_impuesto_pagar_2 = $this->get_isr_impuesto_pagar ($empresa, $ejercicio, $periodo - 2);
	...
	public function	get_coeficiente_acreditamiento ($empresa, $ejercicio, $periodo) {
		...
		return ($coeficiente = $average / $total_ingresos) ? $coeficiente : 0;
	}
	...
	public function isr_perdida_anteriores($empresa, $ejercicio, $periodo) {
		...
		for ($i = $periodo; $i <= 12; $i++) {
			...
			if ($i == 1 || $i == 2) {
				...
				$sql = "SELECT ISNULL(Importes12, '0') AS average
						 ...
						 WHERE IdCuenta 
							   ...
							   AND Ejercicio = (SELECT Id
												  FROM Ejercicios
												 WHERE Ejercicio = (".$ejercicio_obj->nombre." - 2)) --Enero y Febrero, Marzo a Diciembre - 1 en vez de - 2
							   AND Tipo = '1' --Saldo";
			}
			else {
				// SQL
				$sql = "SELECT ISNULL(Importes12, '0') AS average
						 ...
						 WHERE IdCuenta 
							   ...
							   AND Ejercicio = (SELECT Id
												  FROM Ejercicios
												 WHERE Ejercicio = (".$ejercicio_obj->nombre." - 1)) --Enero y Febrero, Marzo a Diciembre - 1 en vez de - 2
							   AND Tipo = '1' --Saldo";
			}
			...
			$data['id_ejercicio'] = $ejercicio;
			$data['id_periodo']   = $i;
			$data['isr_perdida_anteriores'] = $query['average'];
