...
class PagoProvisionalpmController extends Zend_Controller_Action {
		...
		if ( $get_data ) {
			...
			<div style="overflow: auto; margin-bottom: 20px;">
				...
				echo   "<tr>
							...
							for( $i = 1; $i <= 12; $i++ ) {
								echo "<td>".number_format($registro[$i]['isr_perdida_anteriores'], 0)."</td>";
							}
					echo   "<td></td>
						</tr>";
				echo   "<tr class='average-pm'>
	...
	public function	get_coeficiente_acreditamiento ($empresa, $ejercicio, $periodo) {
		...
		//Evaluar si no esta configurado aún
		//Respuesta vacia aplicar el método normal.
		if ( empty($query) ) {
			...
			$coeficiente = ($average / $total_ingresos) ? $average / $total_ingresos : $coeficiente;
		}
		else {	//Con respuesta en alguno de los dos puntos.
  ...
  public function PMT($i, $n, $p) {
	...
	public function exportarAction() {
		...
		if ( $get_data ) {
			...
			foreach ($columns AS $periodo => $letter) {
				$objPHPExcel->objPHPExcel->getActiveSheet()->setCellValue($letter.$i, number_format($registro[$periodo]['isr_perdida_anteriores'], 0));
			}
			$i++;
